...
def new_student():
    ...
    nationality_commbobox.delete(0,tkinter.END)

    numcourses_spinbox.delete(0, tkinter.END)
    numcourses_spinbox.insert(0, "0")

    numsemester_spinbox.delete(0, tkinter.END)
...
def enter_data():
    ...
    if accepted == "accepted" :
        ...
        if firstname and lastname :
            ...
            age, nationality,registration_status, numcourses, numsemester) VALUES (?,?, ?, ?, ?, ?, ?,?)'''
            data_insert_tuple = (firstname, lastname, title,
                                 age, nationality, registration_status, numcourses, numsemester)
            cursor = conn.cursor()
            cursor.execute(data_insert_query, data_insert_tuple)
            conn.commit()   #data kaydeder
            conn.close()
